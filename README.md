...
instructions at the [bottom of this page](#using-seamo-as-a-template).
...
`seamo/seamo.py`. The code provided in this file is intentionally rather
...
We place the module code in a file called `seamo.py` in directory called
`seamo`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import seamo as sb` in an
interactive Python session, the classes and functions defined inside of the
`seamo.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the seamo module, the main classes defined at the bottom of
...
Either way, you can create a `seamo/data` folder in which you can
...
(typically, something like our `seamo/tests/test_seamo.py`). Within each
...
have one such test in `seamo/tests/test_seamo.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/seamo`),
and type:
...
will see a message such as:
    ...
    seamo/tests/test_seamo.py:49: AssertionError
...
To document `seamo` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/seamo/blob/master/doc/_static/RTD-advanced-conf.png)

 http://seamo.readthedocs.org/en/latest/
...
A `seamo/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/seamo) for the software. This
...
`seamo` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `seamo`, we use the
...
[here](https://github.com/uwescience/seamo/blob/master/scripts/Figure1.ipynb).
...
Under "old repository clone URL" enter: https://github.com/uwescience/seamo
...
`seamo/seamo.py` to be called `smallish/smallish.py`

	git mv seamo smallish
	git mv smallish/seamo.py smallish/smallish.py
	git mv smallish/tests/test_seamo.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `seamo` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `seamo` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `seamo` in them. Type
the following to see where all these files are:

	git grep seamo

You can replace `seamo` for `smallish` quickly with:

	git grep -l 'seamo' | xargs sed -i 's/seamo/smallish/g'
...
Edit all the mentions of `seamo` in the `seamo/__init__.py` file, and in
the `seamo/version.py` file as well.
